...
rule all:
	input: ["korea_HGT_candidates_summary.pdf"]

workdir: "korea_outputs"
...
# Initial Orthofinder run with all assemblies and all reference NCBI assemblies
	input:
		protGB="GENOME_PROTEIN_DB_{sample}.pkl"
	output:
		dir=directory("{sample}_OrthoFinderAll_output_directory")
	shell:
		"mkdir {output}"
...
#	output:
#		dir=directory("{sample}_phylotypes_clusters")
#	shell:
#		"mkdir {output}"
...
#	output:
#		dir=directory("{sample}_directory_with_proteins_ordered_by_phylotype")
#	shell:
#		"mkdir {output}"
...
#	output:
#		dir=directory("{sample}_OrthoFinder_output_directory")
#	shell:
#		"mkdir {output}"
...
rule diamond_meganizer:
	input:
		"{sample}_nr_aln_original.daa"
	output:
		report("{sample}_nr_aln_meganized.daa",
...
rule get_hits_metadata:
	...
	output:
		"{sample}.diamond_interesting_hits_metadata.csv"
	shell:
		"touch {output}"
...
rule diamond_plot_interesting_hits:
	input:
		hits="{sample}.diamond_interesting_hits.csv",
		meta="{sample}.diamond_interesting_hits_metadata.csv"
	output:
